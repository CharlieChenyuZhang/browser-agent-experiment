...
async def _initialize_llm(
    ...
    if not provider or not model_name:
        # Fallback to OpenAI if not explicitly configured
        fallback_api_key = os.getenv("OPENAI_API_KEY", "")
        if fallback_api_key:
            provider = "openai"
            model_name = "gpt-4o"
            api_key = fallback_api_key
            # Allow endpoint override via env if provided
            base_url = base_url or os.getenv("OPENAI_ENDPOINT", "https://api.openai.com/v1")
            logger.warning(
                "LLM Provider/Model not set. Falling back to OpenAI (gpt-4o) using OPENAI_API_KEY from environment."
            )
            try:
                ...
                # Inform user in UI that default was used
                gr.Info("Using default LLM: OpenAI gpt-4o (from OPENAI_API_KEY)")
                return llm
            except Exception as e:
                logger.error(f"Failed to initialize default OpenAI LLM: {e}", exc_info=True)
                gr.Warning(
                    f"Failed to initialize default OpenAI LLM. Please set provider/model manually. Error: {e}"
                )
                return None
        else:
            logger.info("LLM Provider or Model Name not specified, and OPENAI_API_KEY not set. LLM will be None.")
            return None
    try:
